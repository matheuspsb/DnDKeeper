...
export function useInitiative() {
  ...
  const addCombatant = useCallback((data: Omit<Combatant, 'id'>) => {
    setState((prev) => {
      ...
      const next = sortedByInitiative([...prev.combatants, { ...data, id: crypto.randomUUID() }])
  ...
  const adjustHp = useCallback((id: string, delta: number) => {
    setState((prev) => {
      const next = prev.combatants.map((combatant) => {
        if (combatant.id !== id || combatant.hp === null || combatant.maxHp === null)
          return combatant
  ...
  const updateInitiative = useCallback((id: string, initiative: number) => {
    setState((prev) => {
      const activeId = currentId(prev)
      const updated = prev.combatants.map((combatant) =>
