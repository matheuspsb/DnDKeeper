...
function NpcDossierRow({
  ...
  return (
    <article className={`border-b border-ink-800/70 ${isDead ? 'opacity-60' : ''}`}>
      ...
      >
        <span className="relative h-14 w-14 shrink-0 overflow-hidden border border-ink-800 bg-ink-800">
          {image ? (
            <img
              src={image}
              alt={npc.name}
        ...
        <span className="min-w-0 flex-1">
          <span className="flex items-center gap-3">
            <span className="truncate font-display text-[17px] font-medium tracking-wide text-bone-100">
              {npc.name}
            </span>
            <NpcStatusStamp status={npc.status} animate={false} />
          </span>
          {npc.description && (
            <span className="mt-1 block truncate font-body text-[14px] leading-normal text-bone-300">
              {npc.description}
            </span>
          )}
        </span>
        ...
        <ChevronRightIcon
          size={18}
          className={`shrink-0 text-bone-400/60 transition-transform duration-200 ${
            expanded ? 'rotate-90' : ''
          }`}
        />
      </button>
      ...
      {expanded && (
        <div className="dossier-open flex flex-col gap-5 pb-6 pl-18 sm:flex-row">
          ...
          <div className="flex min-w-0 flex-1 flex-col gap-4 pr-2">
            {npc.description && (
              <p className="font-body text-[15px] leading-relaxed text-bone-100/90">
                {npc.description}
              </p>
            )}
            ...
            {npc.notes && (
              <div>
                <p className="mb-1.5 font-display text-[11px] tracking-[0.16em] text-brass uppercase">
                  Anotações
                </p>
                <p className="font-body text-[15px] leading-relaxed text-bone-300 italic">
                  {npc.notes}
                </p>
              </div>
            )}

            <p className="font-mono text-[11px] text-bone-400/70">
