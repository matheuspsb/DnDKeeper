...
- **Guard contra PUT desatualizado (`localEditSeq`)**: em produção, com round-trip de rede real, dava pra ver o HP diminuir e depois "voltar" sozinho após cliques rápidos em sequência — um segundo clique podia acontecer **enquanto o PUT do primeiro ainda estava em voo**; quando esse PUT respondia, seu `onSuccess` sobrescrevia o cache com o valor antigo (sem a edição do segundo clique), e o flush seguinte acabava mandando esse valor revertido pro servidor — perda silenciosa do clique, não só flicker visual. Corrigido com um contador `localEditSeq` (`useRef`, incrementado a cada `mutate()`): cada PUT carrega o `seq` vigente no momento do envio, e o `onSuccess` só aplica a resposta se `variables.seq === localEditSeq.current` — se uma edição mais nova já aconteceu enquanto o PUT viajava, a resposta é descartada (o flush dessa edição mais nova é quem vai valer). **Isso não é o mesmo problema do "guard de versão (409)" da seção Pendências** — aquele é sobre dois mestres editando ao mesmo tempo (perda de escrita entre clientes diferentes); este é sobre o próprio cliente pisando na própria edição mais recente.
- `hasPendingLocalChange()`: `true` enquanto há um flush agendado (`scheduleFlush` ainda não disparou) **ou** um PUT em andamento (`push.isPending`) — usado como guard pro SSE (ver abaixo).
- API pública **inalterada** pros demais consumidores: `combatants`, `currentIndex`, `round`, `addCombatant`, `addCombatants`, `removeCombatant`, `adjustHp`, `updateInitiative`, `setConditions`, `setImageUrl`, `setHpRevealed`, `nextTurn`, `goToTop`, `reset`, `saveFailed`.
...
- **Aceita `shouldSkip?: () => boolean`** (guardado num `useLatestRef` pra não recriar o `EventSource` a cada render): quando `true`, o snapshot recebido é ignorado — `connected`/`lastEventAt` continuam atualizando normalmente, só a aplicação do estado no cache é pulada. `/iniciativa` passa `hasPendingLocalChange` do `useInitiative` (o backend faz broadcast de todo `save()` pra **todos** os assinantes, inclusive quem mandou o PUT — sem esse guard, o próprio eco via SSE do mestre podia sobrescrever uma edição local ainda não confirmada, o mesmo tipo de corrida do `localEditSeq` acima mas pelo canal SSE em vez do retorno direto do PUT). `/mesa` não passa nada (é só leitura, nunca tem edição local pendente).
